import { AIMessage, AIResponse, ModelRouterConfig, RoutingRule } from './types';
import { getModelById, getModelsByCapability } from './models';
...
export class ModelRouter {
  ...
  private analyzeQuery(query: string): {
    ...
    let type: 'code' | 'reasoning' | 'creative' | 'analysis' | 'summarization' = 'reasoning';
  ...
  private selectModel(query: string, context?: any): string {
  ...
  async routeQuery(
    ...
    try {
      ...
      if (selectedModel !== this.config.fallbackModel) {
        ...
        const fallbackModel = getModelById(this.config.fallbackModel);
